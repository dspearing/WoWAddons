...
function Ellipsis:UNIT_AURA(unitTag)
	...
	local spellName, _, stackCount, _, duration, expireTime, unitCaster, _, _, spellID = UnitAura(unitTag, 1, filter)
	...
	while (spellName) do -- only scanning our auras on target
		...
		index = index + 1
		spellName, _, stackCount, _, duration, expireTime, unitCaster, _, _, spellID = UnitAura(unitTag, index, filter)
	end
