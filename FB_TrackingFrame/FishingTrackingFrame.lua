...
local function CoinP(fid)
    local _, texture, _, _, _, name = FBAPI:GetFishieRaw(fid);
...
local function ResortFishies(force)
    ...
    if ( not TrackFishies ) then
        local sf = FBAPI:GetSortedFishies();
        TrackFishies = {};
        for idx=1,#sf do
    ...
    if ( not hourlyfish[id] and not weeklyfish[id] ) then
        for how in pairs(PBTF_Info) do
            if ( PBTF_Info[how][id] ) then
                ...
                FBAPI:Print(FB_TFConstants.NOTRACKMSG, name);
            end
        end
    else
        FBAPI:Message(FB_TFConstants.NOTRACKERRMSG);
    end
end
...
local function UpdateGraphData(graph, id, info)
    ...
    PBTF_Info["COLORS"][id] = color;
    local name, texture;
    info.item, texture, _, _, _, name = FBAPI:GetFishie(id);
    if ( texture ) then
...
local function TrackFishToggle(fishid, how)
    ...
    if ( not tab[fishid] ) then
        ...
        if ( tab[fishid].plot == 1) then
            PBTF_Info[how][fishid].nevermore = not FBAPI:GetSettingBool("TrackHidden");
        else
...
FB_TrackingFrame.Update = function(self, forced)
    ...
    for idx=offset,limit do
        ...
        if ( locButton ) then
            ...
            local item, texture, _, _, _, name, _ = FBAPI:GetFishie(fid);
            text = FBAPI:StripRaw(name);
            locButton.item = item;
...
FB_TrackingFrame.OnEvent = function(self, event)
    ...
    FBAPI:RegisterHandlers(TrackingEvents);
    FBAPI:PerFishOptions(TrackOptionsMenu);
    ...
    target = FBAPI:CreateManagedFrameGroup(FBConstants.LOCATIONS_TAB,
    ...
    FBAPI:CreateManagedOptionsTab(target, FBConstants.LOCATIONS_TAB, target.groups, optiontab)
...
local function track_fish(how, fishlink)
    ...
    how == FBConstants.WEEKLY)) then
        ...
        if ( i ) then
            ...
            FBAPI:Print(FB_TFConstants.TRACKINGMSG, n, how);
            return true;
        end
    end
end;

local function untrack_fish(fishlink)
    local c, i, n = FL:SplitLink(fishlink, true);
    if ( i ) then
        UntrackThis(i, n);
        return true;
...
FBAPI:AddCommand(FB_TFConstants.TRACK, track_fish, FB_TFConstants.TRACK_HELP, {"[%w]+", "|c%x+|Hitem:%d+:%d+:%d+:%d+|h%[.-%]|h|r"})
FBAPI:AddCommand(FB_TFConstants.NOTRACK, untrack_fish, FB_TFConstants.NOTRACK_HELP, {"|c%x+|Hitem:%d+:%d+:%d+:%d+|h%[.-%]|h|r"})

local function tracking_help()
    for jdx in pairs(PBTF_Info) do
        local how = trackingWordMap[jdx];
        FBAPI:Print("Tracking "..how);
        local ft = PBTF_Info;
...
FBAPI:AddCommand(FB_TFConstants.TRACKING, tracking_help)
