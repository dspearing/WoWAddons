...
--local CNname3, _, _, _, _, _, _ = EJ_GetInstanceInfo(1195); -- Sepulcher of the First Ones
...
function oilvlgetraidstat()
	...
	cfg.asc = {}
	OilvlGetStatisticId("Dragonflight","Aberrus, the Shadowed Crucible",cfg.asc,true)
end
...
function OGetRaidProgression2(RaidName, OSTAT, NumRaidBosses, Faction)
	...
	--bigorp[CNname3] = Save_orp(CNname3, OSTATCN3, 11)
	...
	--SaveAOTCCE(RaidAchiv[CNname3],15470,15471)
		...
		if oilvlframedata.name[m] ~= ""  and oilvlframedata.name[m] and oilvlframedata.guid[m] ~= "" and oilvlframedata.guid[m] then
			...
			local function CheckGearAvail(n,slot,pp)
				if oilvlframedata.gear[n][slot] then
					...
					elseif oilvlframedata.gear[n][slot][4] == 0 then eg = "|cFF00FFFF"  end-- missing gem
...
local gemTexture = {
	[3743190]=true,[3743188]=true,[3743191]=true, [3743193]=true, [463466]=true, [457655]=true, [1392550]=true, [1003591]=true
}
		...
		if cfg.oilvlms then
			if Omover2 == 1 then
				if UnitExists(rpunit) and CheckInteractDistance(rpunit, 1) and rpsw then
					...
					--if cfg.raidmenuid == 3 then OGetRaidProgression2(CNname3, OSTATCN3, 11, UnitFactionGroup(rpunit)); end
				else
					--ClearAchievementComparisonUnit();
					rpsw=false;
					rpunit="";
			...
			elseif Omover2 == 2 then
				if UnitExists(rpunit) and CheckInteractDistance(rpunit, 1) and rpsw then
					...
					--if cfg.raidmenuid == 3 then OGetRaidProgression3(CNname3, OSTATCN3, 11, UnitFactionGroup(rpunit)); end
				else
					--ClearAchievementComparisonUnit();
					rpsw=false;
					rpunit="";
				...
				if UnitExists("target") and CheckInteractDistance("target", 1)  and rpsw then
					...
					--if cfg.raidmenuid == 3 then OGetRaidProgression(CNname3, OSTATCN3, 11); end
				else
					--ClearAchievementComparisonUnit();
					rpsw=false;
					rpunit="";
...
function OilvlRaidMenu()
	if not ORaidDropDownMenu then
	   --CreateFrame("frame", "ORaidDropDownMenu", cfg.frame, "LibDD:UIDropDownMenuTemplate")
	   --L_Create_UIDropDownMenu("ORaidDropDownMenu", cfg.frame)
		LibDD = LibStub:GetLibrary("LibUIDropDownMenu-4.0")
		local frame = LibDD:Create_UIDropDownMenu("ORaidDropDownMenu", cfg.frame)
	end
...
--  oilvl opacity
	...
	mscb:SetScript("PostClick", function()
		...
		if oilvlsilvl:GetChecked() then
			LibDD:UIDropDownMenu_EnableDropDown(ORaidDropDownMenu)
		else
			LibDD:UIDropDownMenu_DisableDropDown(ORaidDropDownMenu)
		end
	end);
