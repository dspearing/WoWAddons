...
GOALTYPES['get'] = { -- combining old 'get' and 'collect' now.
	...
	iscomplete = function(self)
		...
		if self.targetid2 then got = got + GetItemCount(self.targetid2) end
		if self.targetid3 then got = got + GetItemCount(self.targetid3) end
		...
		for i,goal in pairs(step.goals) do
			if goal.action == "goldcollect" then
				...
				for _,target in ipairs(goal.targets) do
					local numInBags = GetItemCount(target[2]) or 0
			...
			if self:IsComplete() then
				...
				if self.skilllevel then
					...
					if self.recipedata and (self.recipedata.productid or self.recipedata.variants) then
						have = GetItemCount(self.targetid or self.recipedata.productid)
						if self.targetid2 then have = have + GetItemCount(self.targetid2) end
						if self.targetid3 then have = have + GetItemCount(self.targetid3) end
						if have then
						...
						if self.targetid  then have = GetItemCount(self.targetid) end
						if self.targetid2 then have = have + GetItemCount(self.targetid2) end
						if self.targetid3 then have = have + GetItemCount(self.targetid3) end
