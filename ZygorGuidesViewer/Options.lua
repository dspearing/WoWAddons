...
function ZGV:Options_DefineOptionTables()
	...
	do
		...
		if ZGV.IsRetail or ZGV.IsClassicWOTLK then 
		...
		AddOption('',{ type = 'description', name=L['opt_group_notification'], font=ZGV.font_dialog})
			...
			AddOption('n_popup_candrag',{
				...
				set = function(i,v) Setter_Simple(i,v)
					if n_popup_candrag == 1 then
						ZGV.db.profile.toastframe_anchor = nil
					else
						ZGV.NotificationCenter.FloatingToast:CanDrag(true)
						ZGV.NotificationCenter.FloatingToast:SetScript("OnDragStop", function(self)
							self:StopMovingOrSizing()
							ZGV.F.SaveFrameAnchor(ZGV.NotificationCenter.FloatingToast,"toastframe_anchor")
						end)
					end
				end,
			})
		...
		do
			...
			do -- Fake Popups
				...
				AddOption('faketoast_2', { type = 'execute', width=160, 
					...
					--	ZGV.Widgets.Registered.weeklyreset:Initialise()
						ZGV.NotificationCenter:CreateFloatingFrame("message", true, "World Quests have reset on this server.","weeklyreset")
					end
				})
