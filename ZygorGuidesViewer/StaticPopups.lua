...
--	if not self.Queue or #self.Queue < 1 or self.IsPopupVisible then return end
	if not self.Queue or #self.Queue < 1 or (self.IsPopupVisible and (not ZGV.db.profile.n_popup_toast or PopupHandler:OverrideToast())) then return end
...
function PopupHandler:OverrideToast(popuptype)
	...
	--]]
		...
		if ZGV.ItemScore.Upgrades.forcefull or ZGV.GuideProto.forcefull then
			return true
		end
	end
end

function PopupHandler:ReturnToast()
	ZGV.GuideProto.forcefull = false
	ZGV.ItemScore.Upgrades.forcefull = false
	if ZGV.IsClassic or ZGV.IsClassicTBC or ZGV.IsClassicWOTLK then
		ZGV.Skills.forcefull = false
	end
end
...
function PopupHandler:QueuePush(popup)
	...
	if not self.IsPopupVisible or (ZGV.db.profile.n_popup_toast and ZGV.NotificationCenter.FloatingToast and ZGV.NotificationCenter.FloatingToast:IsVisible()) then
		self:QueuePop()
	end
end
