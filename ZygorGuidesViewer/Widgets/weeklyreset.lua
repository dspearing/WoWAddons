...
function widget:Update(forced)		--the argument is for fake toast option
	...
	if ZGV.IsRetail then
		if seconds==0 and ZGV.db.profile.n_popup_msg_worldquest and not ZGV.db.global.bannedtoasts["weeklyreset"] then
		--	ZGV.NotificationCenter:CreateFloatingFrame("message", nil, ZGV.NotificationCenter.MsgTstWorldQuests,"worldquests")
			ZGV.NotificationCenter:CreateFloatingFrame("message", nil, format_text("Weekly Quests have reset on this server."), "weeklyreset")
		end
		...
		--	ZGV.NotificationCenter:CreateFloatingFrame("message", nil, ZGV.NotificationCenter.MsgTstWorldQuests,"worldquests")
			if ZGV.db.profile.n_popup_msg_worldquest and not ZGV.db.global.bannedtoasts["weeklyreset"] then
				ZGV.NotificationCenter:CreateFloatingFrame("message", nil, format_text("Weekly Quests have reset on this server."), "weeklyreset")
			end
			ZGV.db.global.nextreset = C_DateAndTime.GetSecondsUntilWeeklyReset()
		end
